...
from backend.nodes.dfp_db_node import dfp_db_node
...
print(f"DFP DB agent tools: {[tool.name for tool in dfp_db_agent.tool_dict.values()]}")
# Build Graph
...
graph.add_conditional_edges("dfp_db", lambda s: END if s["done"] else "supervisor", {})
